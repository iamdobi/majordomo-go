...
// Mdcli Majordomo Protocol Client API.
type Mdcli struct {
	broker  string
...
//  Send Here is the send method. It sends a request to the broker and gets a
...
func (mdcli *Mdcli) Send(service string, request ...[]byte) (reply [][]byte, err error) {
	//  Prefix request with protocol frames
	//  Frame 1: "MDPCxy" (six bytes, MDP/Client x.y)
	//  Frame 2: Service name (printable string)

	req := make([][]byte, 2, len(request)+2)
	req = append(req, request...)
	req[1] = []byte(service)
	req[0] = []byte(MDPC_CLIENT)
	if mdcli.verbose {
	...
	for retries_left := mdcli.retries; retries_left > 0; retries_left-- {
		...
		if len(polled) > 0 {
			var msg [][]byte
			msg, err = mdcli.client.RecvMessageBytes(0)
			if err != nil {
				break
			}
			if mdcli.verbose {
			...
			if string(msg[0]) != MDPC_CLIENT {
				panic("msg[0] != MDPC_CLIENT")
			}

			if string(msg[1]) != service {
				panic("msg[1] != service")
			}
